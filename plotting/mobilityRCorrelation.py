...
def plotTimeseries(mergedDf, category):
    ...
    ax.plot(x, presence_transposed, label=label)
    # m, b = np.polyfit(x, y, 1)
    # ax.plot(x, m * x + b, '-')
    plt.xlabel('Date')
    plt.ylabel('Rt / Change in presence {}'.format(category))

    plt.title('Correlation between Rt and {}'.format(category))

[mergedDf, corrDf, pDf] = processData(countryDf,reproductionDf)
plotCorrelation(mergedDf, corrDf, pDf, allCategories[5])
# plotTimeseries(mergedDf,allCategories[3])
